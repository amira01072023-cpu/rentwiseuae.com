...
import { calculateLeaveSalary } from '@/lib/calculators/leaveSalary';
...
export default function LeaveSalaryCalculator() {
  ...
  const errors = useMemo(() => {
    ...
    if (leaveDays === '' || Number(leaveDays) <= 0) {
      next.push('Please enter a valid number of leave days.');
    }
    return next;
  }, [basicSalaryMonthly, leaveDays]);

  const result = useMemo(() => {
    return calculateLeaveSalary({
      basicSalaryMonthly: typeof basicSalaryMonthly === 'number' ? basicSalaryMonthly : 0,
      leaveDays: typeof leaveDays === 'number' ? leaveDays : 0,
    });
  }, [basicSalaryMonthly, leaveDays]);
    ...
    <div className="space-y-10">
      <HowToUseTool
        steps={[
          ...
          'Enter the number of leave days you want to estimate.',
          'Use the result as a planning estimate and verify the final value with your employer or HR team.',
        ]}
      />
      ...
      <div className="grid gap-8 lg:grid-cols-[1.15fr_0.55fr] lg:items-start">
        <SurfaceCard className="rounded-[2rem] border border-slate-200 px-8 py-10 shadow-none ring-0">
          <div className="grid gap-8 md:grid-cols-2">
            ...
            <label className="grid gap-3 text-sm text-slate-500">
              <span className="text-xl font-semibold text-slate-500 sm:text-2xl">Leave Days *</span>
              <input
                className="border-0 border-b border-slate-400 bg-transparent px-0 py-3 text-lg text-slate-900 outline-none"
                type="number"
                value={leaveDays}
                onChange={(e) => setLeaveDays(e.target.value === '' ? '' : Number(e.target.value))}
              />
            </label>
          </div>
        ...
        <div className="rounded-[2rem] border-l-[6px] border-blue-700 bg-gradient-to-b from-slate-50 to-white p-8 shadow-sm ring-1 ring-slate-200">
          <div className="text-lg font-semibold uppercase tracking-[0.14em] text-slate-500">Estimated leave salary</div>
          <div className="mt-4 text-4xl font-bold text-blue-700 sm:text-5xl">
            {formatAed(canShowResult ? result.leaveSalaryAmount : 0)}
          </div>
          <p className="mt-4 text-sm leading-6 text-slate-600">
            This is a planning estimate based on monthly basic salary and leave days. Verify the final value with HR or your employer.
          </p>
        </div>
      </div>
      ...
      {canShowResult ? (
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <SurfaceCard className="shadow-none">
            <div className="text-sm text-slate-500">Estimated leave salary</div>
            <div className="mt-2 text-2xl font-bold text-slate-900">{formatAed(result.leaveSalaryAmount)}</div>
          </SurfaceCard>
        </div>
      ) : null}
      ...
      <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-5 max-w-6xl text-sm leading-7 text-slate-700">
          This leave salary calculator is provided as a planning and guidance tool only. It is not legal advice and should not be treated as a final employer calculation. Actual outcomes may vary depending on company policy, contract terms, applicable labor rules, salary treatment, payroll practices, and HR calculations. Always verify the final amount with your employer or HR department.
        </p>
      </div>
    </div>
  );
